...
#
# print("可用显卡：",torch.cuda.device_count())
...
class eeemodelLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        semantic_out  = inputs
        semantic_gt  = targets
        #print(semantic_out.shape,semantic_gt.shape)
...
def run(args):
    ...
    logdir = f'./log/{time.strftime("%Y-%m-%d-%H-%M")}({cfg["dataset"]}-{cfg["model_name"]})'
    ...
    optimizer = Ranger.Ranger(params_list, lr=cfg['lr_start'], weight_decay=cfg['weight_decay'])
    ...
    for ep in range(cfg['epochs']):
        ...
        for i, sample in enumerate(train_loader):
            ...
            if cfg['inputs'] == 'rgb':
                ...
                # print(image.shape)
                # print("train error image name:", sample["label_path"])
                ...
                # binary_label = sample['binary_label'].to(device)
                # targets = [label, binary_label, bound]
                #edge=torch.randn(1,480,640).cuda()
                targets=label
                try:
            ...
            for i, sample in enumerate(test_loader):
                if cfg['inputs'] == 'rgb':
                    ...
                    try:
                        ...
                        print("test error image name:",sample["label_path"])
                        continue
                    # predict = model(image, depth)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default="configs/MCPNet.json", help="Configuration file to use")
    parser.add_argument("--opt_level", type=str, default='O1')
    parser.add_argument("--inputs", type=str, default='rgb', choices=['rgb', 'rgbd'])
